...
if os.path.isdir("boost"):
    pattern = "VERSION " + version.replace(
        ".", "\\."
    )  # XXX: 1.91.0-1 still has 1.91.0 in CMakeLists.txt.
    if (
        subprocess.run(
            ["grep", pattern, "boost/CMakeLists.txt"], check=False
        ).returncode
        != 0
    ):
        # Version mismatch
...
if not os.path.isdir("boost"):
    ensure("tar", ["xJf", f"cache/{boost_tar}", "-C", "."])
    ensure("mv", [f"{boost_dir}", "boost"])

# For js, but harmless for non-windows platform so no need to revert.
sed(
    "boost/libs/container/include/boost/container/detail/thread_mutex.hpp",
...
BOOST_CONTEXT_ABI = {"arm64": "aapcs", "x86_64": "sysv"}.get(MACOS_ARCH)
