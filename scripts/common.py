...
def steal(package: str, directories: tuple[str, ...] = ("share",)):
    ...
    mkdir(directory)
    ensure(tar, ["xjf", f"cache/{prebuilt}", "-C", directory, *directories])
            ...
            if IOS_PLATFORM == "OS64":
                sdk = [
                    ...
                    subprocess.check_output(
                        "xcrun --sdk iphoneos --show-sdk-path", shell=True, text=True
                    ).strip(),
                ]
...
class Builder:
    def __init__(
        ...
        ios: list[str] | None = None,
        harmony: list[str] | None = None,
        src=".",
    ...
    def strip(self):
        ...
        match PLATFORM:
            case "harmony":
                    ...
                    f"{HARMONY_NATIVE}/llvm/bin/llvm-strip",
                    ["--strip-unneeded", *all_a],
                )
            case "js":
    ...
    def package(self):
        ...
        for d in ("share/doc", "share/man"):
            ...
            [
                ...
                "-f",
                f"{self.dest_dir}{POSTFIX}.tar.bz2",
                *glob.glob("*"),
            ],
        )
...
class CMakeBuilder(Builder):
    def configure(self):
        ...
        c_cxx_flags = [
            f"-ffile-prefix-map={os.path.abspath(self.src)}=."
        ]  # Reproducible: __FILE__
        if self.definitions:
        ...
        if PLATFORM == "windows":
            ...
            )  # Reproducible: TimeDateStamp in llvm: WinCOFFObjectWriter.cpp
        ...
        ensure(command[0], [*command[1:], *self.options])
    ...
    def install(self):
        os.environ["DESTDIR"] = self.dest_dir
            ...
            "cmake",
            ["--install", self.build_],
            stdout=subprocess.DEVNULL if self.name == "boost" else None,
        )
...
class MakeBuilder(Builder):
    ...
    def build(self):
        ...
        if PLATFORM == "harmony":
            ...
            command += ["CC=clang", "AR=llvm-ar", "RANLIB=llvm-ranlib"]
        ensure("make", command)
