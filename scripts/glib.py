...
if PLATFORM == "js":
    ...
    # Originally https://github.com/kleisauke/glib
        ...
        "git", ["fetch", "https://github.com/fcitx-contrib/glib", f"{branch}:{branch}"]
    )
...
class GLibBuilder(MesonBuilder):
    def pre_package(self):
        sed(
            f"{self.dest_dir}{INSTALL_PREFIX}/lib/pkgconfig/gio-2.0.pc", "s|-lintl||g"
        )  # kkc
        if PLATFORM == "js":
            sed(
                f"{self.dest_dir}{INSTALL_PREFIX}/lib/pkgconfig/gmodule-no-export-2.0.pc",
                "s|-pthread||g",
            )  # kkc
            sed(
                f"{self.dest_dir}{INSTALL_PREFIX}/lib/pkgconfig/glib-2.0.pc",
                "s|-pthread||g",
            )  # kkc and skk
